...
class PhpStan(lint.Linter):
		...
		if settings.get("use_composer_autoload", True):
			...
			if autoload_file:
				if any(self.autoload_opts(settings)):
					error = "Composer autoload-file conflicts with PHPStan user setting.\n"
					error += 'Disable "use_composer_autoload" or remove "{}" from "args".'.format('", and "'.join(self.autoload_opts(settings)))
					logger.error(error)
					self.notify_failure()
					return []
	...
	def find_autoload_php(self, file_path):
		...
		while file_path:
			...
			if path.isfile(autoload) and (path.isfile(composer_json) or path.isfile(composer_lock)):
				# return autoload
